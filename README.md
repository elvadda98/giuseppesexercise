...
<head>
    ...
    <title>Vocab Practice: Changes & Habits</title>
    <style>
        :root {
...
<header>
    <h1>Vocabulary: Habits & Adjustments</h1>
...
<script>
    const data = [
        { word: "low-season", def: "The time of year when travel is less popular and cheaper.", sent: "I prefer traveling during the ___ because there are fewer crowds.", pron: "Flights are much cheaper in the ___." },
        { word: "rusty", def: "Out of practice in a skill; or covered in iron oxide.", sent: "My English is a bit ___ because I haven't spoken it in years.", pron: "Don't touch that ___ old gate." },
        { word: "replace", def: "To take the place of something; to provide a substitute.", sent: "I need to ___ the batteries in my remote control.", pron: "Can you ___ the broken window?" },
        { word: "used to", def: "In the process of becoming accustomed to something.", sent: "I am slowly getting ___ waking up early for my new job.", pron: "She's not ___ the cold weather yet." }
    ];
    ...
    function init() {
        ...
        pCont.innerHTML = "";
        data.forEach((item, i) => {
            pCont.innerHTML += `<div class="gap-sentence">${item.pron.replace(item.word, '_________')} <button class="mic-btn" onclick="testSpeech('${item.word}', ${i})">🎤</button> <div id="p-fb-${i}" style="color:#888; font-size: 0.9em;">Guess & speak!</div></div>`;
        });
    }
